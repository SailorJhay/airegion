...
export interface AIRegionOptions {
...
export interface AIRegionFile {
    flag: string;
    content?: string;
}

export interface AIRegionBotModification {
    name?: string;
    instructions?: string;
    fileIds?: string[];
}

export class AIRegion {
    ...
    public areonRpcUrl: string;
    public areonProvider: ethers.providers.JsonRpcProvider;
    public receiverAddress: string;
    public debugMode: boolean;

    constructor(options: AIRegionOptions) {
        ...
        this.areonRpcUrl = options.areonRpcUrl;
        this.areonProvider = new ethers.providers.JsonRpcProvider(options.areonRpcUrl);
    ...
    async startAI() {
        ...
        // Start RPC server
        const rpc = new RPC({ ...this.rpcOptions, aiRegionClient: this });
    ...
    async createAssistant(instructions: string, name: string, extraFileIds: string[]): Promise<OpenAI.Beta.Assistants.Assistant> {
        ...
        const assistant = await this.openai.beta.assistants.create({
            instructions: instructions || "You are a Areon blockchain technical support bot. You help answer technical questions. Avoid unrelated questions, you only answer Areon ecosystem related questions. Don't say you can't access the uploaded files.",
            name: name || "AIRegion",
    ...
    async modifyAssistant(assistantID: string, options: AIRegionBotModification): Promise<OpenAI.Beta.Assistants.Assistant> {
    ...
    async uploadFile(fileObj: AIRegionFile, format: string = ".txt"): Promise<{ idCreated: string, fileKeyCreated: string }> {        
