...
export class RPC {
    ...
    public aiRegionClient: AIRegion;
    public db: Level;
    public messageQueue: MessageQueue = new MessageQueue();

    constructor(options: RPCOptions) {
        this.RPC_PORT = options.RPC_PORT || 20297;
        this.aiRegionClient = options.aiRegionClient;
        this.db = this.aiRegionClient.db;
    }
    ...
    async startServer() {
            ...
            switch (req.body.method) {
                case "getAuthKey":
                    ...
                    // Get authkey through signature
                        ...
                        // Verify sig
                        const hash = ethers.utils.solidityKeccak256(["string"], ["AIREGION_AUTH_MESSAGE" + req.body.params.salt]);
                ...
                case "createChatBot":
                    {
                        ...
                        try {
                            ...
                            if (req.body.params.username !== this.aiRegionClient.adminUsername && userInfo.assistantList.length >= plans[subscription].botLimit) {
                                throwError("Bot limit exceeded.", 400);
                                return;
                            }

                            // Create a new openai assistant for user
                            const assistant = await this.aiRegionClient.createAssistant(
                            ...
                            if (subscription === "advanced") {
                                await this.db.put("USAGE_LIMIT" + assistant.id, "150000");
                            }
                            ...
                            if (this.aiRegionClient.debugMode) console.log(e);
                ...
                case "getChatBotInfo":
                    try {
                        ...
                        const assistant = await this.aiRegionClient.getAssistantInfo(req.body.params.assistantID);
                        ...
                        // Get the usage limit, usage limit only exists on paid bots, default is 1500 for every free account
                        let limit = 1500;
                        try {
                ...
                case "deleteChatBot":
                    {
                        ...
                        try {
                            ...
                            const assistant = await this.aiRegionClient.deleteAssistant(req.body.params.assistantID);
                ...
                case "modifyChatBot":
                    {
                        ...
                        try {
                            ...
                            const assistant = await this.aiRegionClient.modifyAssistant(req.body.params.assistantID, {
                ...
                case "purchasePack":
                    if (req.body.params.paywith === "area") {
                        let txReceipt;
                        ...
                        try {
                            txReceipt = await this.aiRegionClient.areonProvider.getTransaction(req.body.params.txHash);
                        } catch (e) {
                            if (this.aiRegionClient.debugMode) console.log(e);
                        ...
                        if (txReceipt.to !== this.aiRegionClient.receiverAddress) {
                        ...
                        if (txReceipt.value.toBigInt() >= ethers.utils.parseEther("1").toBigInt()) {
                            try {
                                ...
                                for (const assistantID of assistantList) {
                                    await this.db.put("USAGE_LIMIT" + assistantID, "150000");
                                }
                ...
                case "uploadFile":
                    {
                        ...
                        // Main process
                        try {
                            const uploadInfo = await this.aiRegionClient.uploadFile(
                                {
                ...
                case "streamAdd":
                    {
                        ...
                        try {
                            // Add chunk to file
                            this.aiRegionClient.streamAdd(req.body.params.fileKey, req.body.params.chunk, fileFormat);
                ...
                case "streamFinish":
                    {
                        ...
                        try {
                            ...
                            const file = await this.aiRegionClient.streamFinish(req.body.params.fileKey, fileFormat);
                ...
                case "getFileInfo":
                    {
                        ...
                        try {
                            ...
                            const file = await this.aiRegionClient.getFileInfo(req.body.params.fileId);
                ...
                case "createChatThread":
                    try {
                        ...
                        const thread = await this.aiRegionClient.createThread();
                    ...
                    try {
                        await new Promise((resolve, reject) => {
                            const queueCallback = async () => {
                                ...
                                if (usage > usageLimit && req.body.params.assistantID !== this.aiRegionClient.initialBotId) {
                                    throwError("Response limit exceeded.", 400);
                                    return;
                                }
    
                                // Create a message
                                const message = await this.aiRegionClient.createMessage(
                                    req.body.params.threadID,
                                    req.body.params.question
                                );
    
                                // Let the assistant reply to the message
                                const run = await this.aiRegionClient.runThread(
                                    req.body.params.threadID,
                ...
                case "getMessages":
                    ...
                    try {
                        ...
                        const messages = await this.aiRegionClient.getMessages(
                            req.body.params.threadID, 
                            req.body.params.assistantID,
                            onlyNew,
                            limit
        ...
        fastify.listen(this.RPC_PORT, "0.0.0.0", (err, address) => {
            ...
            clog(`AIRegion RPC server listening at address ${address}`);
        });
    }
}
