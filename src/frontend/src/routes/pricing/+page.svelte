<script>
    ...
    onMount(() => {
        ...
        let areonProvider = new ethers.providers.Web3Provider(window.ethereum),
            areonSigner;
        ...
        purchaseWithAreon = async function() {
            // Add Areon Network Testnet if it's not already added yet
            ...
            if (accounts.length) {
                areonProvider = new ethers.providers.Web3Provider(window.ethereum);
                areonSigner = areonProvider.getSigner();
            }

            const { chainId } = await areonProvider.getNetwork();

            // Only pay when network is Areon
            if (chainId === 462) {
                const tx = await areonSigner.sendTransaction({
    ...
    let plans =[
            {
                ...
                features:[
                    ...
                    {
                        content:"Cap custom chatbot: 1500/month",
                        status:true
                    },
                    {
            ...
            },
            {
                ...
                features:[
                    ...
                    {
                        content:"Cap custom chatbot: 150000/month",
                        status:true
                    },
                    {
                        content:"Training bot",
...
<!--Static Landing Page-->
<div class="relative flex flex-col max-w-screen-2xl mx-auto bg-dark text-white">
    <NavBar/>
    <!--Content-->
    <div class="relative flex flex-col w-full items-center justify-center h-fit mb-[50px] ">
        <div class="text-center text-sea sm:text-4xl text-2xl font-bold sm:mt-[60px] mt-[40px] mb-[20px]">
            Get the complete AIREGION to your platform
        </div>
        <div class="  w-full flex justify-center py-10">
            ...
            <div class="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 xl:gap-12 lg:gap-6 sm:gap-8 gap-4 sm:w-fit w-full sm:px-0 px-10">
                {#each plans as plan}
                    <!--Plan-->
                    <div class="shrink-on-hover bg-lightDark flex flex-col sm:gap-3 gap-1 p-4 col-span-1 border border-sea shadow-xl rounded-xl lg:px-8 md:px-4">
                        ...
                        <div class="sm:text-xl text-lg flex items-center text-lg">
                                ...
                                <img class="{plan.price === 0? "opacity-0":""} inline-block sm:h-[25px] h-[18px]" src ="/AREA.svg"/>
                                <span class="{plan.price === 0? "opacity-0":""}">/month</span>
                        </div>

                        <!--Feature container-->
                        <div class="flex flex-col border-t border-darkGray pt-4 gap-1">
                        ...
                        <!--Button-->
                        <button class={`mt-5 py-[5px] text-white hover:border-sea  border-2 border-purple rounded-lg ${plan.buttonBg}`} on:click={plan.buttonAction}>
    ...
    {#if isPopupTryNow}
        <div class="absolute font-semibold flex flex-col inset-0 bg-black  bg-opacity-60 z-50" in:fade={{ duration: 200 }} out:fade={{ duration: 200 }}>
            <div class="flex flex-col mx-auto  mt-[200px] md:max-w-[500px] w-full max-w-[400px] h-[280px] bg-lightDark gap-2 rounded-lg">
                <!--Close button + reset page to 1-->
                <button class="self-end"
                        on:click={()=>{isPopupTryNow = false}}>
                    <svg class="h-8 p-2 fill-darkGray hover:fill-sea"  viewBox="0 0 32 32" fill="none">
