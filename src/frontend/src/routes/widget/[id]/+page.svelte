...
<div class="w-full fixed bottom-0 right-0 flex flex-col z-2509 sm:mr-2 mr-0">
        ...
        <div class="max-w-[440px] max-h-[588px] bg-white flex flex-col border rounded-lg shadow-lg" in:fade={{ duration: 300 }} out:fade={{ duration: 300 }} >
            ...
            <div class="flex justify-between items-center mt-2 mx-2 pb-2 border-b-2">
                <div class="flex items-center gap-1">
                    ...
                    {:else}
                        <img class="h-[25px] inline-block" src="/logoWidget.svg"/>
                    {/if}
                </div>
                <div class="flex gap-4 mx-2">
                    ...
                    <button disabled={questionStatus === "loading" || questionStatus ==="waiting"} on:click={()=>{messageList = []}}>
                        <svg class="h-[18px] 
                        {questionStatus === "loading" || questionStatus ==="waiting"?"fill-gray-300":"hover:fill-purple fill-black"} " viewBox="0 0 489.711 489.711" xml:space="preserve">
                    ...
                    <!--Close Button-->
                    <button
                        on:click ={()=>{isShowChat = false; isShowWidget = "default"; window.parent.postMessage("airegion-hide", "*");}}>
                        <svg class="h-[18px] hover:stroke-purple stroke-black" viewBox="-0.5 0 25 25" >
            ...
            <div class="flex flex-col overflow-auto h-[420px] border-b mx-2 my-3 gap-2 sm:min-w-[350px] min-w-[250px]">
                {#each messageList as message}
                    ...
                    {#if message.type === "user"}
                        <div class="self-end max-w-[333px] break-words text-white border mr-3 p-2 rounded-lg bg-purple" >
                            {message.message}
                        </div>
                    {/if}
                {/each}
            ...
            <div class="flex border rounded-lg shadow-lg mx-2 items-center mb-2">
                ...
                <!--Send button-->
                <button class="px-2 {question === "" || !fulfilled || questionStatus === "loading"? 'fill-gray-400':'hover:fill-black fill-purple'}" disabled={question === "" || !fulfilled || questionStatus === "loading"}
    ...
    {:else}
        ...
        {#if isShowWidget === "default" } 
            <button class="self-end my-2 hover:bg-purple bg-opacity-50 rounded-full p-1" in:fade={{ duration: 300 }}  
                on:click ={() => {
                    isShowChat = true;
                    isShowWidget = "close";

                    window.parent.postMessage("airegion-show", "*");
                }}>
        ...
        {#if isShowWidget === "close" } 
            <button class="self-end my-2 hover:bg-purple bg-opacity-50 rounded-full p-1" in:fade={{ duration: 300 }} 
                on:click ={() => {
                    ...
                    window.parent.postMessage("airegion-hide", "*");
                }}>
